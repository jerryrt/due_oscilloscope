...
def build(args):
    ...
    try:
        ...
        for label, key in (("50 kHz", "1"), ("200 kHz", "3"),
            ...
            if key == "5":
                why += (
                    "\n\n**This picture used to carry a defect, and the "
                    "fact that it no longer does is the more useful "
                    "half.** Above 200 kHz every capture lost exactly "
                    "three frames before the first one reached the host "
                    "- on three benches and three hosts, with zero "
                    "growth afterwards - and this shot was published "
                    "with red counters and a caption explaining them.\n\n"
                    "The cause was one line in the wrong order. "
                    "`cmd_stream` printed its banner *after* starting "
                    "the capture; a console print costs 13-20 ms of "
                    "blocked main loop, the ring holds 8.96 ms at this "
                    "rate, and everything past that runway was gone "
                    "before the host saw a byte. Moving the print above "
                    "the start, on both firmware tracks, takes it to "
                    "zero. Issue #41.\n\n"
                    "**It was found by photographing this pane.** The "
                    "number had been on screen for as long as anyone had "
                    "been looking at it and read as ordinary noise; it "
                    "surfaced only because the script that takes these "
                    "screenshots refuses to caption one *clean* while "
                    "any alarm counter is non-zero. This shot is now "
                    "held to that check like every other."
                )
