    ...
    try:
        ...
        for label, key in (("50 kHz", "1"), ("200 kHz", "3"),
            ...
            if key == "5":
                why += (
                    ...
                    "the reason is worth more than the picture.** Above "
                    "200 kHz the capture stream loses a fixed handful "
                    "of frames before the first one reaches the host - "
                    "exactly 3 here, on three benches, at both rates "
                    "above the threshold - and then runs clean for as "
                    "long as anyone has watched.\n\n"
                    "The count not growing with duration is what made "
                    "it a start-up condition rather than a leak, and "
                    "the cause turned out to be one line in the wrong "
                    "order: `cmd_stream` prints its banner *after* "
                    "`stream_start`. A console print costs 13-20 ms of "
                    "blocked main loop - this project prices that "
                    "explicitly, because printf is a debug method and "
                    "not an instrument - the capture ring holds a fixed "
                    "number of frames, and everything past that runway "
                    "is gone before the host ever sees a byte. It is "
                    "issue #41, and the fix is to move the prints ahead "
                    "of the start.\n\n"
                    "**This number had been on this pane for as long as "
                    "anyone had been looking at it, and read as "
                    "ordinary noise.** It surfaced because the script "
                    "that takes these screenshots refuses to caption one "
                    "*clean* while any alarm counter is non-zero. Every "
                    "other picture here is verified against those "
                    "counters immediately before the grab; this one is "
                    "published with the defect showing, because "
                    "restarting the run only incurs it again."
                )
        ...
        if win.session.call("console", text="=900S") is not None:
            ...
            g.shot("16-measure-stalled",
                   ...
                   "The same stall, one panel to the left. A stalled "
                   "loop feeds the DAC nothing, and playback abandons "
                   "itself after 500 ms without a byte rather than "
                   "holding a buffer for ever - so by 900 ms the DAC has "
                   "stopped being driven and holds its last code. What "
                   "is left on A0 is that level plus the bench's own "
                   "noise. Mean and RMS agree with each other exactly, "
                   "which is what a DC level looks like measured "
                   "honestly.\n\n"
                   "**The timing fields refuse, and there are now two "
                   "ways for them to do it - the panel names which.** "
                   "*signal too flat to time* is a last-resort floor on "
                   "peak-to-peak; *no periodic component* is the guard "
                   "that actually rejects noise. Which one you see here "
                   "depends on how much noise this particular run drew, "
                   "and that is not a fixed quantity: measured on this "
                   "bench, the same configuration eight times gave a "
                   "quiet window of about 12 codes in six runs and about "
                   "46 in two.\n\n"
                   "**That drift is why the second guard exists.** The "
                   "floor used to be the only one, at ten codes - about "
                   "8 mV here, against roughly 15 mV of noise sitting on "
                   "an undriven DAC pin. It did not reject noise, it "
                   "rejected *quiet* noise, and the same stalled board "
                   "would refuse at 6 mV and report **9,523.8 Hz at "
                   "1.2 % duty** at 24 mV, to five figures, off nothing "
                   "at all. No amplitude constant could fix that when "
                   "the amplitude it must clear moves by four times "
                   "between runs. The replacement is a **ratio** - how "
                   "peaked the spectrum is, largest bin over median - "
                   "which carries no amplitude and so cannot sit below a "
                   "floor that drifts. Issue #43.",
