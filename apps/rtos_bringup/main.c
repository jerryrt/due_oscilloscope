...
/*
 * `y`. The body is shared now and the letter is in the shared table,
 * which it was not: this track bound it and no other did, so it
 * dispatched while being invisible to `h` and to console_missing().
 * A letter one board answers and another silently ignores, with no way
 * for a host to discover the difference, is the hazard CLAUDE.md names
 * about `T` - wearing a different costume.
 *
 * It still matters most here. millis() and micros() are this
 * application's own (time_rtos.c) because bsp/systick.c cannot be
 * linked, and almost every driver in the tree calls them.
 */
static void c_time(const uint32_t *a) { (void)a; console_cmd_time_check(); }
