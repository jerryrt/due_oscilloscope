 ...
 *
 * Track C is a third track. It is NOT an oracle in invariant 3's
 * sense and must not be read as one: it links Track B's drivers/,
 * bsp/ and lib/ unchanged, so a disagreement between B and C points
 * at main() or at the kernel and never at the register programming,
 * which is the same object file. That is what makes it the right
 * experiment for the question it exists to ask - does a scheduler
 * underneath change the timing of a data path that is otherwise
 * byte-identical - and the wrong one for any question about register
 * programming.
 */
#ifndef TRACK_ID_H
#define TRACK_ID_H
