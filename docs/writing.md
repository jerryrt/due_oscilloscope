...
| **No issue numbers** | Issues carry what is being argued; `docs/` carries what is settled. A document that says "see #48" tells the reader the answer lives somewhere it does not. **Unless the citation is live state and lives nowhere else** - an open objective and a bench's environment are on standing issues, and a reader chasing those does have to go and join them. Where a `docs/` file owns the finding, cite the file |
