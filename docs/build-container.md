...
| **D7** | **`-Werror` on every bench, with one option to turn it off, which the image never uses.** | `-DFIRMWARE_WERROR=OFF`. Free on Track B, which builds 0 warnings on GCC 14.2.1. **Track A's zero is `-w`, not cleanliness**: `cmake/track_a.cmake` silences the vendored core and the same loop reaches the sketch, hiding 11 `-Wunused-variable` in `sketches/bringup/bringup.ino` and disarming every analyser on that track. The escape hatch is for the three-compiler reality - a warning that only 15.2.1 emits must not fail someone else's build while they are mid-diagnosis on something unrelated |
