/*
 * Track A's copy of drivers/playstat.h, and deliberately a copy.
 *
 * Invariant 3: the two toolchains share no source. What they must share
 * is the wire, and docs/control-protocol.md makes that a requirement -
 * the host parses one magic, one layout and one CRC for both tracks, so
 * this file is byte-for-byte the other one and any edit belongs in both
 * on the same day. tests/test_play_counters.py is where a divergence
 * shows up, because it parses these records with no idea which track
 * emitted them.
 */
/*
